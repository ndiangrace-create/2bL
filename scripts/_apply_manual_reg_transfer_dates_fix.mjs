...
function mustRegex(src, re, replacement, label){
...
// 1) 主辦代報名：日期、價格、checkbox 必須在同一張可點擊列，不再被全域 label/grid 樣式拆散。
const oldDate=`      return \`<label style=\\"display:flex;gap:7px;align-items:center;margin:6px 0\\"><input type=\\"checkbox\\" class=\\"amrDate\\" value=\\"\${attr(day)}\\"> \${safe(day)}\${fee?\`｜\${money(fee)}\`:''}</label>\`;`;
const newDate=`      return \`<label class=\\"amr-date-option\\" style=\\"display:grid!important;grid-template-columns:28px minmax(0,1fr) auto!important;align-items:center!important;gap:10px!important;width:100%!important;padding:12px 14px!important;margin:7px 0!important;border:1px solid var(--line)!important;border-radius:14px!important;background:#fff!important;cursor:pointer!important\\"><input type=\\"checkbox\\" class=\\"amrDate\\" value=\\"\${attr(day)}\\" style=\\"width:20px!important;height:20px!important;margin:0!important;justify-self:start!important\\"><span style=\\"min-width:0;font-weight:900;color:var(--ink)\\">\${safe(day)}</span><strong style=\\"white-space:nowrap;color:var(--main2)\\">\${fee?money(fee):'依場次費用'}</strong></label>\`;`;
admin=mustReplace(admin,oldDate,newDate,'manual registration date row');

// 2) 轉場：目標場次為多日時必須可勾選實際轉入日期；單日自動勾選。
const transferStart=admin.indexOf('async function openRegistrationTransferPanel(regId,sid){');
const transferEnd=admin.indexOf('async function confirmRegistrationCredit(regId,sid){');
if(transferStart<0||transferEnd<0||transferEnd<=transferStart) throw new Error('missing transfer ui block');
...
function selectedResolutionTargetDates(){return [...document.querySelectorAll('.resolution-target-date:checked')].map(x=>String(x.value||'').slice(0,10)).filter(Boolean);}
async function refreshTransferPreview(regId,sid,resetDates=false){
  const targetSessionId=$('resolutionTarget')?.value||'';
  const dateBox=$('resolutionTargetDates');
  const calc=$('resolutionCalc');
  const submit=$('resolutionSubmit');
  ...
  try{
    ...
    const p=normalizedResolutionPreview(raw,regId);
    ...
    if(dateBox){
      dateBox.innerHTML=options.length?options.map(o=>{
    ...
    if(!effective.length){if(calc)calc.innerHTML='這個場次沒有可用日期，無法計算。';return;}
    // 單日第一次載入時，第一次 preview 沒有帶日期；用自動勾選的正式日期再算一次。
    ...
    const finalP=normalizedResolutionPreview(finalRaw,regId);
    if(calc)calc.innerHTML='<b>選擇日期：</b>'+effective.map(safe).join('、')+'<br><b>新場應繳：</b>'+money(finalP.targetTotal||0)+'（活動費 '+money(finalP.targetActivityFee||0)+'＋押金 '+money(finalP.targetDeposit||0)+'）<br>'
      +'<b>已轉入：</b>'+money(finalP.appliedTotal||0)+'｜<b>應補差額：</b>'+money(finalP.dueAmount||0)+'<br>'
      +'<b>轉後活動金：</b>'+money(finalP.creditCreated||0)+'｜<b>待退押金：</b>'+money(finalP.depositRefundDue||0)+'<br><span class="sub">設備不沿用；新報名只保存本次勾選日期。</span>';
...
`;
admin=admin.slice(0,transferStart)+transferBlock+admin.slice(transferEnd);

// 3) Worker：targetDates 只能來自目標場次正式 dates_json；未傳時維持舊相容＝全選。
const workerStart=worker.indexOf('async function hPreviewRegistrationResolution(env,b){');
const workerEnd=worker.indexOf('async function hPartialDayRefund(env,b){');
if(workerStart<0||workerEnd<0||workerEnd<=workerStart) throw new Error('missing worker resolution block');
