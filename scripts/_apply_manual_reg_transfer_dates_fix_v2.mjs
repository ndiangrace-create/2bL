...
// A. 主辦代報名：只換日期選擇的 UI 片段，資料收集 .amrDate / selectedDates 完全沿用。
...
// B. 轉場 UI：增加正式目標日期勾選；不改活動金、退款等原入口。
...
// C. Worker：selected targetDates 只接受目標 sessions.dates_json 正式日期；沒傳時維持舊 API 相容（全部日期）。
...
// D. 只為未來「新建立的轉場報名」補每日營運 rows；trigger 與原 RPC 同 transaction，無歷史回填。
...
console.log('manual registration / multi-day transfer patch v2 applied');
