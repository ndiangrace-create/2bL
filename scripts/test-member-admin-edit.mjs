...
ok(auth.includes("'adminUpdateMemberProfile'"),'central authorization missing');ok(index.includes('const phone=(silent&&verifiedPhone)?verifiedPhone:requestedPhone;'),'silent save must preserve verified phone');ok(index.includes('背景儲存只能沿用已驗證手機'),'background phone guard missing');ok(index.includes('你正在修改登入用手機'),'explicit phone confirm missing');
